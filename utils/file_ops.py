...
class FileOps:
    ...
    def list_files(self, directory: str, extensions: Optional[List[str]] = None) -> List[str]:
        ...
        # Perform the listing
        return self.file_ops.list_files(directory, extensions)
...
class PluginSandboxedFileOps:
    ...
    def _check_path_permissions(self, path: str, write_access: bool = False) -> bool:
        """
        Check if a path is allowed for access by this plugin
        ...
        # Check if path is within any of the safe paths
        for safe_path in self.safe_paths:
            if abs_path == safe_path or abs_path.startswith(safe_path + os.sep):
                # If write access is required, only allow in plugin_dir and config_dir
                if write_access:
                    if abs_path == self.plugin_dir or abs_path.startswith(self.plugin_dir + os.sep) or \
                       abs_path == self.config_dir or abs_path.startswith(self.config_dir + os.sep):
                        return True
                    else:
                        self.log(f"Write access denied to {abs_path}", "WARNING")
                        return False
                else:
                    return True
        
        # Path is not in any safe location
        self.log(f"Access denied to {abs_path}", "WARNING")
        return False
    ...
    def read_file(self, file_path: str, encoding: str = 'utf-8') -> Tuple[bool, str]:
        ...
        # Perform the read
        return self.file_ops.read_file(file_path, encoding)
